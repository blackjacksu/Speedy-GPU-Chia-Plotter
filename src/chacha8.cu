...
void get_chacha8_key(struct chacha8_ctx **_x, uint64_t *_pos, uint32_t *_n_blocks, uint8_t **_c, int array_size)
{
    ...
    cudaMalloc((void**) &n_blocks, array_size * sizeof(uint32_t));
    cudaMalloc((void**) &x, array_size * sizeof(struct chacha8_ctx));

    cudaMemcpy(pos, _pos, array_size * sizeof(uint64_t), cudaMemcpyHostToDevice);
    cudaMemcpy(n_blocks, _n_blocks, array_size * sizeof(uint64_t), cudaMemcpyHostToDevice);
    cudaMemcpy(x, _x, array_size * sizeof(struct chacha8_ctx), cudaMemcpyHostToDevice);
    ...
    Kernel_Print<<<gridSize, blockSize>>>(block_dim, thread_id, grid_dim);
